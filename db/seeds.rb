...
datas = Rails.root.join('lib', 'seeds')

Dir.glob(datas.join('**')) do |year|
    csv_data = File.read(year)
    matchday = 1
    i = 1
    CSV.parse(csv_data, :headers => true) do | row |
      ...
      Match.create(
        ...
        home_team: row.to_hash['HomeTeam'],
        home_score: row.to_hash['FTHG'],
        away_team: row.to_hash['AwayTeam'],
        away_score: row.to_hash['FTAG'],
        championnat: ligue1
      )
